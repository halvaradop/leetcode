...
We compare characters from both ends of the string using two pointers (left and right). If the characters match, we move inward. If they don’t match, we try skipping one character — either s[left] or s[right] — and check if the remaining substring is a valid palindrome.
